...
static int ds1374_wdt_release(struct inode *inode, struct file *file)
{
	...
	/*
	 *  Check whether close the watchdog timer.
	 */
	if (expect_close == WDT_OK_TO_CLOSE) {
               	ds1374_wdt_disable();
		pr_info("ds1374_wdt: The timer was stopped!\n");
	} else {
		pr_info("ds1374_wdt: Unexpected close, not stopping!\n");
		ds1374_wdt_ping();
        }
	expect_close = 0;
							...
							unsigned long arg)
{
	...
	case WDIOC_SETOPTIONS:
		...
		if (options & WDIOS_DISABLECARD) {
			if (!nowayout) {
				pr_info("ds1374_wdt: Turn off the watchdog timer.\n");
				ds1374_wdt_disable();
				expect_close = WDT_OK_TO_CLOSE;
				return 0;
			} else {
				pr_info("Nowayout prevents watchdog being stopped!\n");
				return 0;
			}
		}

		if (options & WDIOS_ENABLECARD) {
			pr_info("ds1374_wdt: Turn on the watchdog timer.\n");
			ds1374_wdt_settimeout(timer_margin);
			ds1374_wdt_ping();
			return 0;
		}
...
static int __init ds1374_wdt_init(void)
{
	...
	if (timer_margin != DEFAULT_MARGIN)
                timer_margin *= 4096; 
        pr_info("ds1374_wdt: Watchdog timer initial timeout=%d seconds, nowayout=%d \n",
                        timer_margin/4096, nowayout);
